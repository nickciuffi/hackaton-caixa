...
@ActiveProfiles("test")
@SpringBootTest
 class ConsultarSimulacoesDataProdIntegracaoTest {
    ...
    @Test
    void consultarSimulacoesPorDataEProdSemParcelasIntTest() throws Exception {

        String dataAtual = LocalDate.now().format(DateTimeFormatter.ofPattern("dd-MM-yyyy"));

        MvcResult resJson = mockMvc.perform(get("/simulacao/prod-data?codigoProduto=2&dataReferencia=" + dataAtual + "&mostrarParcelas=0")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(""))
        ...
        SimulacaoPorDataEProdResponseDTO res = objectMapper.readValue(resJson.getResponse().getContentAsString(), new TypeReference<ApiResponse<SimulacaoPorDataEProdResponseDTO>>() {}).getResponse();
        assertEquals(dataAtual, res.getDataReferencia());
        assertEquals(2, res.getSimulacoes().size());
        assertEquals(VALOR_DESEJADO, res.getSimulacoes().get(0).getValorTotalDesejado());
        assertNull(res.getSimulacoes().get(0).getParcelas());
    }

    @Test
    void consultarSimulacoesPorDataEProdComParcelasIntTest() throws Exception {

        String dataAtual = LocalDate.now().format(DateTimeFormatter.ofPattern("dd-MM-yyyy"));

        MvcResult resJson = mockMvc.perform(get("/simulacao/prod-data?codigoProduto=2&dataReferencia=" + dataAtual + "&mostrarParcelas=1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(""))
                .andExpect(status().isOk())
                .andReturn();
        ...
        assertEquals(2, res.getSimulacoes().size());
        assertEquals(VALOR_DESEJADO, res.getSimulacoes().get(0).getValorTotalDesejado());
        assertNotNull(res.getSimulacoes().get(0).getParcelas());
        assertEquals(PRAZO, res.getSimulacoes().get(0).getParcelas().size());
    }
